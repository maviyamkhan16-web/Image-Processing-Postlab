# Digital Image Processing Lab Portal (N-PECCS502P)
**Department of Computer Science & Engineering, S.B.J.I.T.M.R., Nagpur**

## 🔗 Project Links
- **Live Deployment URL:** https://image-processing-postlab-jbnwhfpmwzk7syrbe8lsnj.streamlit.app/
- **GitHub Repository:** https://github.com/maviyamkhan16-web/Image-Processing-Postlab

## 📱 Mobile App QR Code
Scan to open the interactive application directly:

![QR Code](deployment_qr.png)

## 🧪 Experiments Included
- Practical 1: Introduction, Reading & Displaying Images
- Practical 2: Color Formats, Arithmetic & Bitwise Operations
- Practical 3: 2-D Geometric Transformations (Translation, Rotation, Scaling, Reflection, Shearing, Cropping)
- Practical 4: Spatial Enhancements, Histogram Equalization & Thresholding
- Practical 5: Spatial Filtering (Averaging, Gaussian, Median, Bilateral)
- Practical 6: Restoration & Inpainting (Telea and Navier-Stokes)
- Practical 7: Lossy vs Lossless Compression Analysis (JPEG/PNG)
- Practical 8: Morphological Operations (Erosion, Dilation, Opening, Closing)
- Practical 9: Object Detection via Correlation (Template Matching)
- Post-Lab 1: Frequency Domain Filtering (2D-DFT Magnitude Spectrum)
- Post-Lab 2: Color Spaces Analysis (RGB, HSV, YCrCb, CIELAB)
